...
class filter_pumukit extends moodle_text_filter {
    ...
    public function filter($text, array $options = array()) {
        ...
        if (stripos($text, '<iframe') !== false) {
            ...
            $newtext = preg_replace_callback($search, 'filter_pumukit_openedx_callback', $text);
...
function filter_pumukit_openedx_callback($link) {
    global $CFG;
    ...
    $extra_arguments = array(
        ...
        'hash' => create_ticket_openedx($mm_id, $email ? $email : "", parse_url($link[1], PHP_URL_HOST))
    );
...
function create_ticket_openedx($id, $email, $domain) {
    global $CFG;
